...
<pre lang="bash"><code> ```curl -sO https://packages.wazuh.com/4.12/wazuh-install.sh``` </code></pre>
<pre lang="bash"><code> ```sudo bash ./wazuh-install.sh --all-in-one``` </code></pre>
<pre lang="bash"><code> ```sudo cp template/authd.pass /var/ossec/etc/``` </code></pre>
